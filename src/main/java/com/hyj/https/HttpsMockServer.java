...
public class HttpsMockServer extends HttpsMockBase {
    ...
    private static void doHttpsShakeHands(Socket s) throws Exception {
        ...
        hash=clientHash;
        System.out.println("the client hash suanfa is :"+clientHash);

        //第二步，发送服务器证书到客户端
        byte[] certificateBytes=CertifcateUtils.readCertifacates();
        privateKey=CertifcateUtils.readPrivateKeys();
        System.out.println("certifiacte send to client ,the length is :"+certificateBytes.length);
        System.out.println("certification content is :" + byte2hex(certificateBytes));
        SocketUtils.writeBytes(out, certificateBytes, certificateBytes.length);

        System.out.println("get randon number encryted by publicKey");
        int secureByteLength=in.readInt();
        byte[] secureBytes=SocketUtils.readBytes(in, secureByteLength);

        System.out.println("the randon number from client :"+byte2hex(secureBytes));
        byte secureSeed[]=decrypt(secureBytes);
        System.out.println("deencryted random number is:" +byte2hex(secureSeed));

        //第三步 获取客户端加密字符串
        int skip=in.readInt();
        System.out.println("encryed message from client is ：" +skip);
        byte[] data=SocketUtils.readBytes(in,skip);

        System.out.println("encryed message from client : " +byte2hex(data));
        System.out.println("deencryed by privateKey ，count hash values by sha1");
        byte message[] =decrypt(data,new SecureRandom(secureBytes));
        byte serverHash[]=cactHash(message);


        System.out.println("get has values from client");
        int hashSkip=in.readInt();
        byte[] clientHashBytes=SocketUtils.readBytes(in,hashSkip);
        System.out.println("client hash values is : " + byte2hex(clientHashBytes));

        System.out.println("verify client and server hash values");
        boolean isHashEquals=byteEquals(serverHash,clientHashBytes);
        System.out.println("the result is ： " + isHashEquals);
